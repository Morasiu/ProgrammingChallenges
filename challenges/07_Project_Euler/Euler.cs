...
class Euler {
	...
	static void Euler1() {
		...
		//Ok, that's clear, we need to make some efficient for loop.
	...
	static void Euler4() {
		...
		for (int i = 999; i >= 100; i--) {
			for (int j = 999; j >= 100; j--) {
				var num = i * j;
				var isPal = MathHelper.IsPalindrome(num);
				if (isPal && num > pal)
					pal = num;
			}
		}
	...
	static void Euler5() {
		...
		int solution = 20;
		while (true) {
			if (MathHelper.IsDivisible(solution))
				break;
			solution++;
		}
	...
	static void Euler8() {
      ...
      71636269561882670428252483600823257530420752963450";
		allDigits = allDigits.Replace(Environment.NewLine, "");
		...
		Console.WriteLine("Solution: " + digits + " Product: " + maxProduct);
	}
